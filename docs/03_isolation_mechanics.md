...
У 2014 році Amazon запустив **AWS Lambda** — FaaS до широкого хайпу: розробник завантажує функцію, хмара тримає ресурси на нулі, а при виклику піднімає ізольоване середовище, відпрацьовує запит за ~100 мс і гасить.

#### Що таке «холодний старт» (Cold Start)
Коли приходить перший запит або навантаження зростає, готового екземпляра функції в пам'яті немає. Провайдер змушений «з нуля» виділити ресурси хоста, запустити середовище виконання (ОС та runtime), завантажити код користувача і лише потім обробити подію. 

Ця первинна затримка ініціалізації називається **холодним стартом (Cold Start)**. Якщо сам розрахунок триває 50 мс, а середовище піднімається 15 секунд — FaaS втрачає будь-яку практичну цінність для вебу та API.

#### Дилема інженерів AWS
У 2014 році перед архітекторами стояло лише **два класичні шляхи**, і обидва були тупиковими:

| Варіант індустрії | Межа ізоляції | Час старту (Cold start) | Оверхед RAM | Чому не підійшов для Lambda |
| :--- | :--- | :--- | :--- | :--- |
| **Традиційні VM** (EC2, Xen) | Повна апаратна (власне ядро ОС) | 10–40 секунд | сотні МБ | Надійно, але cold start у десятки секунд вбиває latency FaaS |
| **Звичайні контейнери** (Docker) | Логічна (спільне ядро хоста) | ~100 мс | Майже 0 | Швидко, але спільне ядро в multi-tenant — це ризик витоку пам'яті |

### Кейс: як витягнути чужу пам'ять і як від цього захищає ОС

У публічній хмарі на одному залізі запускається код конкурентів, платіжних систем та анонімних користувачів. Якщо сусід читає чужу RAM — хмарна безпека зникає.

#### Чому це катастрофічно небезпечно
Оперативна пам'ять — це єдине місце, де дані лежать **відкритим текстом**. Диск можна зашифрувати LUKS, а трафік — mTLS, але процесор вміє рахувати лише незашифровані байти. У RAM осідають:
- Приватні ключі RSA/ECDSA та SSL-сертифікати.
- Свіжі JWT-токени та відкриті сесії користувачів.
- Паролі до баз даних і системні облікові записи (Cloud API Keys).
- Персональні дані клієнтів (PII, номери карток) безпосередньо під час обробки.

#### Як штатно захищає операційна система
За замовчуванням ядро та процесор тримають пам'ять під суворим контролем:
- **Апаратний блок MMU (Memory Management Unit):** Процес не знає фізичних адрес пам'яті. Він працює у власному віртуальному адресному просторі. Процесор перетворює їх на фізичні через таблиці сторінок (**Page Tables**, регістр `CR3`). Чужих сторінок у таблиці просто не існує.
- **Апаратний бар'єр (`SIGSEGV`):** Спроба прочитати неіснуючу або чужу адресу викликає апаратне переривання процесора (**Page Fault**). Ядро миттєво відправляє процесу `SIGSEGV` (Segmentation Fault) і примусово його вбиває.
- **Занулення при виділенні (Zero-fill on demand):** Коли процес віддає сторінку пам'яті, а ядро пізніше виділяє її іншому (`mmap` / `brk`), ОС обов'язково **перезаписує сторінку нулями**. Наступний процес не прочитає залишки чужих паролів.
- **Блокування міжпроцесного читання (Seccomp / Capabilities):** Контейнери позбавлені права викликати `ptrace` (налагодження іншого процесу) та `process_vm_readv` (пряме читання пам'яті з чужого адресного простору).

#### Як зловмисник обходить захист ОС
Попри віртуальну пам'ять, існують три реальні механізми викрадення:

1. **Експлойт спільного ядра (Kernel Privilege Escalation):**
   - Усі контейнери хоста спираються на **одне ядро Linux**.
   - У разі помилки в ядрі (як-от стан гонитви в copy-on-write **Dirty COW** / CVE-2016-5195 або баг у верифікаторі **eBPF**) атакувальник піднімає привілеї до `Ring 0`.
   - З рівня ядра зловмисник отримує доступ до фізичних адрес RAM без обмежень: можна здампити пам'ять хоста і всіх контейнерів-сусідів.
2. **Апаратні атаки процесора через кеш (Spectre & Meltdown):**
   - Процесори оптимізують швидкість через **спекулятивне виконання** (передбачення розгалужень). Процесор заздалегідь читає дані з пам'яті ще до перевірки прав доступу.
   - Якщо перевірка прав блокує читання, результат операції відкидається, але прочитаний байт **залишається в кеш-пам'яті CPU** (L1/L2/L3).
   - Атакувальник запускає таймінг-атаку (**Flush + Reload**): заміряє час зчитування власного масиву. Швидкий відгук означає, що певний рядок кешу зайнятий чужим байтом. Так біт за бітом «витягується» вміст чужої оперативної пам'яті через фізичний кеш процесора.
3. **Хибні привілеї (Privileged Containers):**
   - Запуск контейнера з прапорцем `--privileged` дає доступ до `/dev/mem` або `/proc/kcore`, дозволяючи читати фізичну пам'ять хоста як звичайний файл.

#### Інженерне рішення Amazon: новий клас MicroVM (Firecracker)
Amazon відмовився обирати «повільно або діряво». Інженери створили власний мінімалістичний монітор віртуальних машин на Rust — **Firecracker** (VMM, *Virtual Machine Monitor*):
- **Викинули 95% емуляції:** позбулися застарілих драйверів QEMU (IDE, флоппі, миші, шини PCI), залишивши мінімальний набір `virtio` (мережа, блочний диск).
- **Апаратна межа (KVM + EPT/NPT):** кожна функція запускається у власній **MicroVM** з персональним ядром Linux. Дворівнева апаратна трансляція сторінок унеможливлює витік пам'яті сусіда навіть при 0-day зламі гостьового ядра.
- **Подолання компромісу:** холодний старт скоротився до **~5 мс**, а оверхед — до **~5 МБ RAM**. Інженери отримали легкість контейнера разом із повноцінною ізоляцією віртуальної машини.

Головний висновок: ізоляція завжди вимагає балансу між безпекою, щільністю пакування та затримкою cold start.

### Еволюція лімітів: від 15 до 90 хвилин у FaaS

![AWS Lambda 90-хвилинні таймаути](./assets/aws_lambda_90m.png)

Роками жорсткий стельовий ліміт AWS Lambda становив **15 хвилин**. Це було усвідомленим компромісом: середовище MicroVM створювали для ультракоротких stateless-функцій та швидкої утилізації «дірок» у пам'яті хоста. Довгі обчислення навмисно виштовхували в окремі контейнери (ECS, Fargate).

Розширення таймауту до **90 хвилин** (Lambda Managed Instances) фіксує зміну ринку:
- **Тиск навантажень:** важкий ETL, батч-обробка та AI/ML інференс потребують серверлесу без утримання постійних кластерів.
...
- **Архітектурний зсув:** межа між суто подієвим FaaS і класичним Batch/Grid остаточно стирається безпосередньо на рівні платформи.
