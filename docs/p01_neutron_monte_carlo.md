...
3. **Вбудований веб-сервер та Dashboard (`HTTPServer`):** Приймає параметри з браузера, запускає симуляцію та повертає JSON із траєкторіями для візуалізації на 2D Canvas.
...
Прискорення перестає зростати і починає падати. Завий виклик нових процесів створює накладні витрати на переключення контексту (Context Switching) в ОС та конкуренцію за ресурси кпу і кешу, оскільки фізичних ядер для одночасного виконання додаткових потоків немає.
</details>
