...
## 1. Оверлейні мережі (Overlay Networks) & CNI

У класичному дата-центрі кожен сервер має свою фізичну IP-адресу. У хмарі на одному сервері запускаються сотні контейнерів, кожен із яких вимагає ізольованого мережевого стеку.

```mermaid
flowchart TD
    subgraph Host1 ["Фізичний вузол 1 (IP: 192.168.1.10)"]
        C1["Контейнер 1<br>(10.244.1.5)"] --- VETH1["veth1"]
        VETH1 --- BR1["Linux Bridge (cbr0)"]
        BR1 --- VTEP1["VXLAN Tunnel Endpoint (VTEP)"]
    end

    subgraph Host2 ["Фізичний вузол 2 (IP: 192.168.1.11)"]
        C2["Контейнер 2<br>(10.244.2.8)"] --- VETH2["veth2"]
        VETH2 --- BR2["Linux Bridge (cbr0)"]
        BR2 --- VTEP2["VXLAN Tunnel Endpoint (VTEP)"]
    end

    VTEP1 ===|"Фізична мережа L3 (Інкапсуляція VXLAN: UDP 4789)"| VTEP2

    style Host1 fill:#1e293b,color:#fff
    style Host2 fill:#1e293b,color:#fff
    style VTEP1 fill:#064e3b,color:#fff
    style VTEP2 fill:#064e3b,color:#fff
```

### Як будується зв'язність:
1. **veth pair (Virtual Ethernet):** Віртуальна «пара кабелів». Один кінець поміщається всередину Network Namespace контейнера як `eth0`, інший залишається на хості та вмикається у міст.
2. **Linux Bridge (`cbr0` / `docker0`):** Працює як віртуальний L2-комутатор, з'єднуючи контейнери однієї машини.
3. **VXLAN (Virtual Extensible LAN):** Технологія оверлейної мережі. Вона бере вихідний Ethernet-фрейм контейнера (L2) і загортає його в стандартний UDP-пакет (L4, порт 4789) фізичної мережі. Фізичні свитчі бачать звичайний трафік між серверами, а контейнери бачать єдину пласку мережу.
4. **CNI (Container Network Interface):** Стандартизований інтерфейс плагінів (Calico, Flannel, Cilium), які налаштовують ці інтерфейси при створенні кожного поду.

### Еволюція маршрутизації: від iptables до eBPF
* **iptables:** Історичний механізм. Плагін створює тисячі правил фільтрації. При 10 000 сервісів перебір правил стає лінійним $O(N)$, викликаючи колосальний оверхед ядра.
* **IPVS (IP Virtual Server):** Використовує хеш-таблиці ядра Linux. Пошук маршруту виконується за сталий час $O(1)$.
* **eBPF (Cilium):** Революційний підхід. Байт-код виконується прямо в ядрі на рівні мережевих сокетів (`sockops`), передаючи дані з одного сокета в інший **взагалі без проходження повного стеку TCP/IP та віртуальних мостів**.
