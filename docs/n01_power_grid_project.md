...
*Граф топології IEEE-118 (режим s=0): колір вузлів показує напругу/навантаження, лінії — рівень завантаженості активним перетоком.*

**Сценарій 2** — інший розподіл навантаження (`s=1`):

![Сценарій 2: операційний стан s=1](./assets/powergraph/ieee118_scenario_02.png)
*Граф топології IEEE-118 (режим s=1): змінений розподіл перетоків потужності між підстанціями.*

**Сценарій 3** — ще один варіант (`s=2`):

![Сценарій 3: операційний стан s=2](./assets/powergraph/ieee118_scenario_03.png)
*Граф топології IEEE-118 (режим s=2): приклад альтернативного добового графіка споживання.*

**Статистика датасету** — після `python source/dataset_scripts/visualize_powergraph.py --data Datasets`:

| Файл | Що показує |
| :--- | :--- |
| `dns_histogram.png` | Розподіл недовідпущеної потужності `dns_MW` по всіх 122 500 сценаріях |
| `blackout_bar.png` | Співвідношення стабільних станів (`output_features = 0`) та блек-аутів (`1`) |
| `branch_flow_vs_capacity.png` | Співвідношення потоку до пропускної здатності кожної лінії (зразок `s=0`) |
| `node_loads.png` | Генерація (додатна) та споживання (від'ємне) активної потужності $P_{net}$ по вузлах |

#### 1. Розподіл дефіциту потужності (DNS)

![Розподіл DNS у PowerGraph IEEE-118](./assets/powergraph/dns_histogram.png)
*Рис. 1. Гістограма значень недовідпущеної потужності `dns_MW`. Переважна більшість сценаріїв зосереджена біля нуля (енергосистема локалізувала аварію); довгий правий хвіст відповідає рідкісним масштабним каскадам.*

#### 2. Співвідношення стабільних станів і блек-аутів

![Бінарні мітки блек-ауту](./assets/powergraph/blackout_bar.png)
*Рис. 2. Співвідношення класів у датасеті (`of_bi`): зелений стовпчик — безпечні режими (DNS = 0), червоний — сценарії, де каскад призвів до аварійного відключення навантаження (DNS > 0).*

#### 3. Навантаженість ліній: потік vs пропускна здатність (`branch_flow_vs_capacity.png`)

![Потік vs ємність ліній, зразок s=0](./assets/powergraph/branch_flow_vs_capacity.png)
*Рис. 3. Завантаження 186 гілок мережі перед аварією (стан `s=0`).*

**Як читати цей графік:**
- **Кожна фіолетова точка** — це одна зі 186 ліній електропередач.
- **Вісь X (`line rating lr (capacity)`):** гранична пропускна здатність лінії (термічний ліміт проводу).
- **Вісь Y (`|P_ij| (initial flow)`):** фактичний потік активної потужності через лінію у вихідному стані до аварії.
- **Пунктирна лінія `flow = capacity` ($y = x$):** поріг спрацювання аварійного релейного захисту:
  - **Усі точки лежать нижче пунктиру:** у нормальному стані жодна лінія не перевантажена, мережа має робочий запас міцності.
  - **Вертикальний ряд на $X \approx 0.72$:** це типові високовольтні магістралі мережі IEEE-118, які мають стандартизований ліміт потужності.
  - **Стовпчик ліворуч з від'ємними значеннями ($X < 0$):** у фізиці від'ємної пропускної здатності не буває. Це внутрішньостанційні трансформатори та масивні збірки шин, які в інженерній моделі IEEE вважаються «необмеженими» (без фіксованого ліміту $lr = 0$). Після Z-score нормалізації датасету PowerGraph нульові ліміти змістилися у від'ємну зону ($-0.18$).
  - **Що з ними робить симулятор:** щоб реле помилково не вимкнуло таку лінію в першу ж мілісекунду симуляції, код призначає їй робочий резерв: `capacity = max(1.0, 2.0 * abs(flow))`, тобто лінія витримає подвійне перевантаження перед аварійним відключенням.

#### 4. Баланс генерації та навантаження по вузлах

![Профіль навантаження вузлів, зразок s=0](./assets/powergraph/node_loads.png)
*Рис. 4. Баланс активної потужності $P_{net}$ у 118 вузлах мережі. Стовпчики вгору ($P_{net} > 0$) — вузли-генератори (електростанції). Стовпчики вниз ($P_{net} < 0$) — вузли-споживачі (міста та підстанції).*
...
```mermaid
flowchart LR
    E1["Етап 1: MIMD (CPU ПК)<br>multiprocessing & GIL"] --> E2["Етап 2: Grid-кластер<br>Ray на фізичних ПК (LAN)"]
    E2 --> E3["Етап 3: Docker & Services<br>Compose + Web UI + Worker"]
    E3 --> E4["Етап 4: Cloud & Orchestration<br>Kubernetes / Cloud Jobs"]
...
**Референс:** структура [`source/mimd-pc`](https://github.com/vplanto/cloud_grid/tree/main/source/mimd-pc) і опис у [Практиці 1](./p01_neutron_monte_carlo.md#3-вихідний-код-та-архітектура-mimd).

- **Мета:** перенести патерн MIMD з практики 1 на симуляцію каскадів IEEE-118; виміряти speedup на `multiprocessing.Pool` в межах однієї машини.

- **Очікувана структура коду** (етап 1):
...
- **Задачі студента:**
  1. Реалізувати `build_topology()` та базовий однопотоковий `run_trial()` з евристикою каскаду (§2.9).
  2. Винести пакет траєкторій у `simulate_trials_chunk()`; розпаралелити через `multiprocessing.Pool.map`.
  3. Додати `run_headless_benchmark()`: фіксація часу, throughput (trials/sec), частка блек-аутів та топ вразливих гілок.
  4. Побудувати графік прискорення (speedup vs `num_workers`) та оцінити закон Амдала.

- **Результат:** `source/grid-mimd/` + `benchmark_results_mimd_grid.json` + графік speedup.

---

### Етап 2. Грід-кластер на фізичних машинах (Ray Grid без Docker)

**Референс:** концепція гетерогенного кластера з [Практики 2](./p02_ray_grid.md) та архітектура HPC/Grid з [Лекції 2](./02_hpc_grid_vs_cloud.md).

- **Мета:** об'єднати **мінімум 2–3 фізичні пристрої** (ноутбуки в аудиторії або домашній LAN: Linux, Windows, macOS) у спільний обчислювальний **Ray Grid**.
- **Архітектура:**
  - **Head-ноут:** координатор кластера (`ray start --head`), що тримає топологію мережі та роздає задачі через `ray.remote`.
  - **Worker-ноутбуки:** підключаються через LAN (`ray start --address='<HEAD_IP>:6379'`), віддаючи свої CPU-ядра під обчислення.
- **Задачі студента:**
  1. Адаптувати виклик `simulate_trials_chunk` під Ray tasks (`@ray.remote`).
  2. Перевірити роботу симулятора при різній потужності машин (гетерогенність вузлів, закон Густафсона-Барсіса).
  3. Дослідити накладні витрати передачі топології по мережі через `ray.put(topology)`.
  4. Провести стрес-тест відмовостійкості: примусово вимкнути один worker-ноутбук під час симуляції великого батчу і перевірити перезапуск тасок.

- **Результат:** `source/grid-ray/` + порівняльний звіт throughput (локальний ПК vs розподілений Ray-грід).

---

### Етап 3. Контейнеризація та сервісна архітектура (Docker Compose + UI)

**Референс:** архітектурні шаблони з [Лекції 3](./03_isolation_mechanics.md) та веб-панель керування з Практики 1.

- **Мета:** упакувати симулятор у стандартизовані контейнери, ізолювати середовище та додати графічний дашборд моніторингу каскадів.
- **Стек:**
  - **Worker service:** масштабований контейнер із розрахунковим рушієм `GridSimulationEngine`.
  - **Backend API (FastAPI):** приймає параметри симуляції (`POST /api/run`, `POST /api/reset`), роздає задачі через чергу.
  - **Message Broker (Redis):** буферизація завдань на масовий прогін Монте-Карло.
  - **Frontend / Dashboard:** візуалізація графа IEEE-118, статус знеструмлених ліній, індикатори сумарного DNS і ризику колапсу.
- **Результат:** `docker-compose.yml` із запуском всього комплексу однією командою (`docker compose up --build`).

---

### Етап 4. Хмарна оркестрація та відмовостійкість (Kubernetes / Cloud Jobs)

**Референс:** оркестрація ресурсів з [Лекції 7](./07_k8s_orchestration_resources.md) та парадигма FinOps з [Лекції 10](./10_finops_resilience_synthesis.md).

- **Мета:** перенести розрахунки у хмарне середовище (Kubernetes, Ray on K8s або Cloud Batch), забезпечивши еластичне масштабування під $10^6 \dots 10^7$ траєкторій.
- **Задачі студента:**
  - Створити маніфести `Deployment` / `Job` для воркерів з лімітами ресурсів (`requests` / `limits`).
  - Налаштувати автоскейлінг обчислювальних подів залежно від глибини черги задач (KEDA / HPA).
  - Провести фінальний масовий розрахунок для виявлення топ-5 критичних ліній енергосистеми IEEE-118.
- **Результат:** робочий пайплайн у кластері, підсумкова аналітична панель та фінальний захист проєкту.
