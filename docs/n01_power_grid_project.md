...
- **Вузол (bus)** є абстрактною **вершиною графа** мережі з числовим ідентифікатором (1…118). Замість географічних координат стан вузла визначають електричні параметри: активне навантаження $P$, напруга $V$ та супутній вектор `B_f_tot`. У фізичній топології за вершиною закріплена підстанція або генератор, проте в моделі це розрахунковий вузол із числовим навантаженням.
- **Гілка (branch)** є **ребром графа** між двома вузлами (лінія електропередачі або силовий трансформатор). Її фіксують парою індексів `(from, to)` у списку `bList` та характеристиками активного потоку $P_{ij}$ і межі нагріву/перевантаження $lr_{ij}$ у масиві `E_f_post`.
...
Симуляція спирається на **топологічний граф**, де структурна зв'язність поєднана з електрофізичними атрибутами. Датасет PowerGraph надає цю топологію разом із чисельним бейзлайном від симулятора Cascades, а власний код моделює наслідки випадкових збурень методом Монте-Карло.

> [!IMPORTANT]
> **Методологічний вибір: топологічна евристика замість розв'язання рівнянь Кірхгофа**
> 
> У промислових пакетах (MATPOWER, Cascades, PSS/E) розрахунок кожного кроку аварії спирається на нелінійну систему рівнянь балансу потужностей (**AC Power Flow**, метод Ньютона-Рафсона) або на лінеаризоване матричне наближення (**DC Power Flow** через інверсію матриці провідностей $\mathbf{B}$ і коефіцієнти розподілу аварійних перетоків **LODF / PTDF**). Через хвильову природу змінного струму відключення однієї ділянки в реальній енергосистемі здатне миттєво перевантажити лінію за десять вузлів від епіцентру.
> 
...
| **$\gg 5$** (напр. $k=50$) | Одномоментне знищення понад чверті мережі; системний колапс стає детермінованим ($P(\text{blackout}) \to 1$) |
...
**Особливості візуалізації:** Графічні схеми в §2.5 демонструють **вихідний операційний стан системи до аварії** (навантаження вузлів і резерв ліній). На демонстраційному дашборді (етап 3) первинне N-k збурення позначається одним кольором, а каскадні вимкнення — іншим. Для перевірки роботи інтерфейсу обирайте сценарії з умовою `cascade_trips > 0`.
...
| Масовий стохастичний експеримент | 122 500 детермінованих розрахунків | Власна генерація $10^5 \dots 10^7$ випробувань |
...
> Файли формату `.mat` мають значний обсяг (~2 ГБ на конфігурацію). Не додавайте їх до git-репозиторію; зберігайте локально у теці `~/Downloads/dataset/`.
...
### 2.9. Топологічна евристична модель для `run_trial()` (Проксі-каскад)

Датасет PowerGraph розраховано за допомогою повної моделі **Cascades** із матричним розв'язанням законів Кірхгофа. У межах лабораторного курсу студенти реалізують **швидкий топологічний проксі-симулятор**:

1. **Ініціалізація:** Завантаження параметрів `Bf[s]`, `Ef[s]`; усі 186 гілок активні (`active = True`).
2. **N-k збурення:** Випадкове вимкнення $k$ ліній (`active = False`). Параметр $k$ обмежений (§1.4). Відбір ліній здійснюється зважено за величиною завантаження `flow/capacity` (§1.5).
3. **Евристичний перерозподіл:** Потік відключеної гілки розподіляється між активними сусідніми лініями пропорційно їхній вільній пропускній здатності ($capacity - flow$). Ця евристика передає динаміку лавинних перевантажень без обчислювально важкого розв'язання рівнянь балансу потужностей.
4. **Ітерація каскаду:** Доки в мережі є лінії з умовою $flow > capacity$, спрацьовує захист (перехід в `active = False`) з повторенням кроку 3.
5. **Критерій зупинки:** Відсутність нових перевантажень, падіння понад $X\%$ сумарного навантаження або досягнення ліміту ітерацій `max_steps`.
6. **Фіксація результатів:** Метрики динаміки (`cascade_trips`, `cascade_steps`, `failed_branch_ids[]`) та агреговані системні наслідки (`dns_proxy`, `blackout`).

**Алгоритм евристичного перерозподілу потоку (деталізація кроку 3):**

Для кожної щойно відключеної гілки $e = (u, v)$ із потоком $F_e$:

1. **Пошук активного оточення:** Сформувати множину активних суміжних ліній $\mathcal{N}_e$, які інцидентні до вузла $u$ або $v$ (крім самої $e$) і мають $\text{active} = \text{True}$.
2. **Перевірка ізоляції (радіальний відвід / тупик):** Якщо $\mathcal{N}_e = \emptyset$ (лінія вела до периферійного вузла або всі сусіди вже відключені), перерозподіл неможливий: потік $F_e$ додається до сумарного недовідпуску потужності (`dns_proxy`), а обробка цієї гілки завершується.
3. **Розрахунок вільного резерву:** Для кожної лінії $i \in \mathcal{N}_e$ визначити доступний запас $R_i = \max(0,\, \text{capacity}_i - \text{flow}_i)$ та їхню суму $R_{\text{total}} = \sum_{i \in \mathcal{N}_e} R_i$.
4. **Перенесення навантаження ($\Delta F_i$):**
   - **За наявності резерву ($R_{\text{total}} > 0$):** потік ділиться пропорційно вільному запасу кожної лінії (більше приймає менш завантажена):
     $$\Delta F_i = F_e \cdot \frac{R_i}{R_{\text{total}}}, \quad \text{flow}_i \leftarrow \text{flow}_i + \Delta F_i$$
   - **За відсутності резерву ($R_{\text{total}} = 0$):** якщо всі сусіди вже завантажені на 100% або більше, аварійний потік ділиться порівну:
     $$\Delta F_i = \frac{F_e}{|\mathcal{N}_e|}, \quad \text{flow}_i \leftarrow \text{flow}_i + \Delta F_i$$
5. **Фіксація відключення:** Встановити $\text{active}_e = \text{False}$ та $\text{flow}_e = 0$. Передати оновлені потоки на перевірку спрацювання захисту в кроці 4 ($\text{flow}_i > \text{capacity}_i$).

**Об'єкт дослідження:** Алгоритм орієнтований на реєстрацію **ланцюгової аварії** (послідовності аварійних відключень після первинного збурення). Поодинокий обрив на периферійному відгалуженні ізолює окремий вузол без розростання аварії (`cascade_trips = 0`). Такі сценарії є локальними.

| Рівень аналізу | Метрика | Фізичний зміст |
| :--- | :--- | :--- |
| **Динаміка процесу** | `cascade_trips`, `cascade_steps` | Масштаб ланцюгової реакції: скільки ліній відключилося після первинного збурення |
| **Системний наслідок** | `dns_proxy`, `blackout` | Частка недовідпущеної потужності в межах усієї топології (еталон у PowerGraph: `dns_MW`) |

Показники $P(\text{blackout})$ та DNS відбивають системний колапс мережі. Рейтинг найбільш уразливих ділянок формується на основі їхнього внеску в ланцюгові аварії.

**Контрольне порівняння (Sanity-check):** Гістограма власного показника `dns_proxy` зіставляється з еталонним розподілом `dns_MW` із датасету PowerGraph на спільних вихідних режимах $s$.

### 2.10. Інженерні нюанси реалізації

- **Співвідношення вибірок:** 122 500 записів датасету є фіксованими робочими режимами генерації та споживання. Власний симулятор обирає базові режими $s$ і запускає поверх них від $10^5$ до $10^7$ випадкових випробувань N-k комбінацій.
...
- **Шкала DNS:** Значення `dns_MW` лежить у діапазоні від 0 до 0.2, оскільки виражає нормалізовану частку недовідпущеної енергії, а не абсолютну потужність у мегаватах. Власні метрики узгоджуються з цим масштабом.
...
- **Особливості візуалізації:** Зображення розділу §2.5 ілюструють нормальний режим роботи мережі. Для перевірки UI обирайте сценарії зі зваженим збуренням (§1.5) та позитивним значенням `cascade_trips > 0`.
...
Скрипти зчитування та візуалізації PowerGraph IEEE-118. Датасет — локально після розпакування [Figshare-архіву](https://figshare.com/articles/dataset/PowerGraph/22820534) (див. §2.2); нижче `--data ~/Downloads/dataset`.
...
python source/dataset_scripts/load_powergraph_cascades.py \
  --data ~/Downloads/dataset --no-large
```
...
```bash
python source/dataset_scripts/load_powergraph_cascades.py \
  --data ~/Downloads/dataset --samples 0:10
```
...
python source/dataset_scripts/visualize_grid_graph.py \
  --data ~/Downloads/dataset --samples 0,1,2
```
