...
| **2. RAG** | У промпт підтягуються чанки з документів | Пошук за **схожістю тексту**, не за узгодженістю: у один промпт потрапляють і застарілий, і актуальний фрагмент; дрібні деталі з «середини» довгого контексту модель часто ігнорує (*lost in the middle*) |
