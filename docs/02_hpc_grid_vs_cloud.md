...
У [лекції 1](./01_flynn_taxonomy.md) ми класифікували **залізо** (SISD — *Single Instruction, Single Data*; SIMD — *Single Instruction, Multiple Data*; MIMD — *Multiple Instruction, Multiple Data*). Далі — навіщо збирають його в кластери й хмари.

[Лекція 0](./00_intro.md) показала ланцюжок мейнфрейм → Грід → хмара. Кожен крок відповідає на інший тип задач; тут формалізуємо різницю.

[Розминка 1](#питання-для-розминки-1-фура-з-вінчестерами-чи-оптика) вже дала відчуття: **фура** виграє за throughput, **оптика** — за latency. Нижче — словник термінів на весь матеріал.
...
Архітектура кластера, вибір планувальника (Slurm чи Kubernetes) і патерн коду (`COPY FROM` проти мільйона `INSERT`) — відповіді на **NFR**, а функціональна специфікація каже лише *що* порахувати.
...
> **Throughput** і **latency** — різні NFR. Один канал не оптимізує одночасно передачу 50 ПБ і доставку одного КБ «прямо зараз» (див. [розминку 1](#питання-для-розминки-1-фура-з-вінчестерами-чи-оптика)).
...
> [!NOTE]
> **Головна ідея лекції:** «найкращого» планувальника не буває. Є зв'язка **профіль навантаження** (batch vs interactive) → **метрика успіху** (throughput vs latency, P99) → **механізм розподілу** (DRF у Slurm vs bin-packing у Kubernetes).
...
Типовий приклад — **аналіз ДНК** (секвенування WGS/WES): FASTQ → вирівнювання (BWA, minimap2) → variant calling (GATK, DeepVariant) → анотація.

**Масштаб:** один зразок **WGS** (людина, ~30× покриття) — **~100 ГБ** FASTQ, job на **16–64 CPU**, **64–128 GB RAM**, **12–48 год** walltime. **WES** (екзом) менший на порядок: **~10–20 ГБ** FASTQ, **8–32 CPU**, **32–64 GB RAM**. Когорта **500 зразків** — **тисячі CPU·доб** і **десятки ТБ** диска. Університетський кластер під Slurm: **50–500 вузлів**, **2–20 тис. ядер**, **ПБ-клас** сховища. Pipeline заводять у чергу як DAG; один HTTP-запит на когорту з сотень зразків тут не проходить.

На кластерах і в лабораторіях такі графи зазвичай пишуть у **Nextflow** (екосистема [nf-core](https://nf-co.re/)) або **Snakemake**; рідше — CWL, WDL або **Galaxy**. Запуск — **Slurm** або хмара (AWS Batch, Google Life Sciences), часто в контейнерах Singularity/Docker.
...
- **Airflow** — оркестратор **даних і ETL** (Python, cron-розклад, веб-UI); зазвичай крутиться «над» K8s або Celery, рідко безпосередньо на Slurm.
...
Якщо **C** впав на вузлі — batch-система **перезапускає** задачу (retry) або валить весь DAG. Клієнт у браузері цього не побачить: у batch немає клієнта в реальному часі.
...
Це **ізоляція** interactive-сервісів один від одного. Batch-адміністратор у тій же ситуації скаже: «займи вузол і рахуй до кінця».
...
[Dashboard реактора](./p01_neutron_monte_carlo.md): браузер кожні 100 мс шле `POST /api/step`. На тому ж вузлі без `cgroups` (control groups — групи контролю CPU/RAM у Linux) крутиться headless-прогін на **10⁶ trials** з `num_workers = всі ядра`.
