...
| **2** | **Тема 2.** Порівняльний аналіз паралелізму в Linux: процеси (`fork`), потоки (`pthread`), пам'ять CoW, IPC канали. | **8** | edX / Linux Foundation: *Linux System Administration Basics* (LFS101x) / DeepLearning.AI: *Distributed Computing with Ray*. | Benchmark однопотокового vs багатопроцесорного (`multiprocessing.Pool`) виконання. Звіт про оверхед IPC. |
