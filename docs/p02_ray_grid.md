...
flowchart TB
    ...
    subgraph W2 ["Вузол 3 — WORKER (Debian Linux)"]
        ...
        W2CPU["Ядра CPU воркера<br>(наприклад, 2 ядра x86_64 з AVX)"]
        C2 --- W2CPU
    end
...
- **Апаратні вимоги:** обов'язкова підтримка процесором інструкцій **AVX** (Advanced Vector Extensions) для роботи Ray;
...
> [!WARNING]
> **«Біль Гріда» на практиці: Апаратна несумісність (AVX та «Помилкова інструкція»):**
> У теорії Грід об'єднує будь-які машини, включно зі старими комп'ютерами кафедри. На практиці сучасні розподілені бібліотеки постачаються як C++ бінарники (wheel), оптимізовані під векторні інструкції **AVX**.
> 
> **Симптом на застарілому залізі:**
> Якщо запустити `python run.py local` (або `import ray`) на процесорах без AVX (старі **AMD Athlon 64 / Athlon II**, Phenom, Intel Core 2 Duo або старі Celeron), Linux миттєво вбиває процес сигналом `SIGILL`:
> ```text
> Помилкова інструкція (або: Illegal instruction (core dumped))
> ```
> 
> **Перевірка процесора (за 3 секунди):**
> ```bash
> grep -m1 -o -E 'avx[0-9]*' /proc/cpuinfo || echo "AVX ВІДСУТНІЙ"
> ```
> 
> **Дії для лабораторної:**
> 1. Вузол без AVX **не може бути вузлом Ray** (ні Head, ні Worker). Зібрати Ray без AVX із джерел займає години і не має сенсу для пари.
> 2. Щоб виконати розрахунки на такому ПК для перших двох етапів (`pool` та `numpy`), запустіть тест із прапорцем пропуску Ray:
>    ```bash
>    python run.py local --skip-ray
>    ```
> 3. Для повноцінного 3-вузлового кластера замініть цей ПК на машину з AVX або запустіть воркер на Head-ноутбуці через `--force`.
